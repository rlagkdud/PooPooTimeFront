...
function paintRecord(hour, min, sec) {
  ...
  if (li.value > 600) {
    newHref = `/html/detail5.html?hour=${hour}&min=${min}&sec=${sec}`;
  } else if (li.value > 300) {
    // 300< <=600
    newHref = `/html/detail4.html?hour=${hour}&min=${min}&sec=${sec}`;
  } else if (li.value > 120) {
    //120< <=300
    newHref = `/html/detail3.html?hour=${hour}&min=${min}&sec=${sec}`;
  } else if (li.value > 60) {
    //60< <=120
    newHref = `/html/detail2.html?hour=${hour}&min=${min}&sec=${sec}`;
  } else {
    // <60
    newHref = `/html/detail1.html?hour=${hour}&min=${min}&sec=${sec}`;
  }
  a.appendChild(li);
  a.id = newId;
  a.href = newHref;
